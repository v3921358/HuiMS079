...
public class AdminCommand{
    ...
    public static class 拉 extends CommandExecute{
        @Override
        public boolean execute(final MapleClient c, final String[] splitted) {
            for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append("!拉 [角色id] - 拉玩家到自己地图").toString();
        }
    }
    public static class 拉全部 extends CommandExecute{
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append("!拉全部 - 拉所有玩家到自己地图").toString();
        }
    }
    ...
    public static class 刷 extends CommandExecute{
        ...
        public boolean execute(final MapleClient c, final String[] splitted) {
            ...
            if (GameConstants.isPet(itemId)) {
                final MaplePet pet = MaplePet.createPet(itemId, MapleInventoryIdentifier.getInstance());
                if (pet != null) {
                    MapleInventoryManipulator.addById(c, itemId, (short) 1, c.getPlayer().getName(), pet, 90);
                }
            }else if (!ii.itemExists(itemId)) {
                c.getPlayer().dropMessage(5, itemId + " - 物品不存在");
            }else {
                ...
                item.setGMLog(c.getPlayer().getName());
                MapleInventoryManipulator.addbyItem(c, item);
                if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"指令通知开关")) <= 0) {
