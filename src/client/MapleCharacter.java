...
public class MapleCharacter extends AbstractAnimatedMapleMapObject implements Serializable{
    ...
    public void silentEnforceMaxHpMp() {//MaplePacketCreator
    ...
    public void gainGachaExp() {
        ...
        if ((currentgexp + exp) >= GameConstants.getExpNeededForLevelS(level)) {
            expgain += GameConstants.getExpNeededForLevelS(level) - exp;
            int nextneed = GameConstants.getExpNeededForLevelS(level + 1);
        ...
        gainExp(expgain, true, false, false);
    ...
    public void changeJob(final int newJob) {
        try {
            ...
            if (normal) {
                this.dropMessage("由于您的转职等級非普通转职等級，技能点异常后果自負。");
            }
            ...
            this.stats.setMaxHp((short)maxhp,this);
            this.stats.setMaxMp((short)maxmp,this);
            this.stats.setHp((int)(short)maxhp,this);
            this.stats.setMp((int)(short)maxmp,this);
    ...
    public void playerDead() {
        if (this.getMapId() != 109020001) {
            ...
            if (statss != null) {
                ...
                this.getStat().setHp(this.getStat().getMaxHp() / 100 * statss.getX(),this);
                this.setStance(0);
    ...
    public void gainExp(final int total, final boolean show, final boolean inChat, final boolean white) {
        try {
            ...
            final int 骑士团等级上限 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"骑士团等级上限"));
            final int 冒险家等级上限 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"冒险家等级上限"));
            if (total > 0) {
    ...
    public void gainExpMonster(int gain, final boolean show, final boolean white, final byte pty, int wedding_EXP, int Class_Bonus_EXP, int Equipment_Bonus_EXP, int Premium_Bonus_EXP, int skillId, int mobId, int mobLv, long mobHp,int 活动经验) {
        ...
        int total = gain + Class_Bonus_EXP + Equipment_Bonus_EXP + Premium_Bonus_EXP + wedding_EXP + 活动经验;
        int partyinc = 0;
        final int prevexp = this.getExp();
        if (pty > 1) {
            partyinc = (int) (((float) (gain * (pty + 1) / 20.0)));
            total += partyinc;
        }
        if (total > 2100000000) {
                total = 2100000000;
        ...
        if (gain != 0) {
            ...
            this.updateSingleStat(MapleStat.EXP, this.getExp());
            if (show) {
                client.sendPacket(MaplePacketCreator.GainEXP_Monster(gain, white, wedding_EXP, partyinc, Class_Bonus_EXP, Equipment_Bonus_EXP, Premium_Bonus_EXP, 活动经验));
            }
            this.stats.checkEquipLevels(this, total);
        }
    }
    ...
    public final void expirationTask(final boolean firstLoad) {
        this.expirationTask(false, firstLoad);
    }
	...
	final List<Triple<MapleInventoryType, IItem, Boolean>> toberemove = new ArrayList<>();
	final List<IItem> tobeunlock = new ArrayList<>();
	for (final MapleInventoryType inv : MapleInventoryType.values()) {
		...
		if (ItemFlag.LOCK.check(item.getFlag())) {
                    ...
                    toberemove.add(new Triple<MapleInventoryType, IItem, Boolean>(inv, item, true)); // 添加删除的道具信息
		}
            } else if (item.getItemId() == 5000054 && item.getPet() != null && item.getPet().getSecondsLeft() <= 0) { // 宠物道具信息
		toberemove.add(new Triple<MapleInventoryType, IItem, Boolean>(inv, item, false));
            }
            }
	}
	for (Triple<MapleInventoryType, IItem, Boolean> itemz : toberemove) {
            IItem item = itemz.getMid();
            if (item == null) {
        ...
        for (IItem itemz : tobeunlock) {
            ...
            dropMessage(5, "封印道具[" + ii.getName(itemz.getItemId()) + "]封印时间已过期。");
        }
	...
	for (ISkill skil : toberem) {
        ...
        if (stat != null && stat.getCustomData() != null && Long.parseLong(stat.getCustomData()) < currenttime) {
            ...
            /*long totalSeconds = lastExpirationTime / 1000;//取到系统时间总秒
            //求出现在的秒
            long currentSecond = totalSeconds % 60;//取到系统时间当前多少秒
            //求出现在的分
            long totalMinutes = totalSeconds / 60 + 1000 * 60 * 20;
            long currentMinute = totalMinutes % 60;//取到系统时间当前多少分
            //求出现在的小时
            long totalHour = totalMinutes / 60 + 1000 * 60 * 20;
            long currentHour = totalHour % 24;
            System.err.println("遍历时限道具时间："+ lastExpirationTime +"");
    ...
    public void forceUpdateItem(IItem item, boolean updateTick) {
        ...
        client.getSession().write(MaplePacketCreator.modifyInventory(updateTick, mods));
    }
    ...
    public void levelUp() {
        ...
        if (this.isGM() || (this.job != 0 && this.job != 1000 && this.job != 2000 && this.job != 2001 && this.job != 3000) || this.level > 9) {
            ...
            this.stats.setStr((short)(this.stats.getStr() + this.remainingAp),this);
            this.remainingAp = 0;
    ...
    public void DoLevelMsg() {
        final int 升级快讯 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"升级快讯开关"));
    ...
    public void modifyCSPoints(final int type, final int quantity, final boolean show) {
        switch (type) {
            case 1: {
                ...
                if (Acash + quantity < 0) {
                    if (show) {
                        this.dropMessage(-1, "目前点券已满，无法获得更多的点券");
                    }
                    return;
                }
            ...
            case 2: {
                if (this.maplepoints + quantity < 0) {
                    if (show) {
                        this.dropMessage(-1, "目前抵用券已满，无法获得更多的抵用券.");
                    }
                    return;
                }
    ...
    public void cancelFairySchedule(final boolean exp) {
        ...
        if (exp) {
            final int 精灵吊坠 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"精灵吊坠经验加成"));
            this.fairyExp = (byte)精灵吊坠;
        }
    }
    
    public void 人气经验加成() {
        final int 人气 = this.getFame() * (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"人气经验加成"));
    ...
    public void resetStats(final int str, final int dex, final int int_, final int luk) {
        ...
        total -= str;
        this.stats.setStr((short)str,this);
        total -= dex;
        this.stats.setDex((short)dex,this);
        total -= int_;
        this.stats.setInt((short)int_,this);
        total -= luk;
        this.stats.setLuk((short)luk,this);
    ...
    public void setHp(final int amount) {
        this.getStat().setHp(amount,this);
    }
    
    public void setMp(final int amount) {
        this.getStat().setMp(amount,this);
    }
    
    public void setMaxHp(final int amount) {
        this.getStat().setMaxHp((short)amount,this);
    }
    
    public void setMaxMp(final int amount) {
        this.getStat().setMaxMp((short)amount,this);
    }
