...
public class NPCConversationManager extends AbstractPlayerInteraction{
    ...
    public void maxStats() {
        ...
        this.c.getPlayer().getStat().setStr((short)32767,c.getPlayer());
        this.c.getPlayer().getStat().setDex((short)32767,c.getPlayer());
        this.c.getPlayer().getStat().setInt((short)32767,c.getPlayer());
        this.c.getPlayer().getStat().setLuk((short)32767,c.getPlayer());
        this.c.getPlayer().getStat().setMaxHp((short)30000,c.getPlayer());
        this.c.getPlayer().getStat().setMaxMp((short)30000,c.getPlayer());
        this.c.getPlayer().getStat().setHp(30000,c.getPlayer());
        this.c.getPlayer().getStat().setMp(30000,c.getPlayer());
    ...
    public void setExpiration(final Object statsSel, final long expire) {
        if (statsSel instanceof Equip) {
            ((Equip)statsSel).setExpiration(System.currentTimeMillis() + (long)(expire * 24L * 60L * 60L * 1000L));
        }
    }
                ...
                for (final MapleCharacter mch : cserv1.getPlayerStorage().getAllCharacters()) {
                    if (数量 >= 0) {
                        ...
                        if ((type.equals((Object)MapleInventoryType.EQUIP) && !GameConstants.isThrowingStar(物品ID) && !GameConstants.isBullet(物品ID)) || (type.equals((Object)MapleInventoryType.CASH) && 物品ID >= 5000000 && 物品ID <= 5000100)) {
                            ...
                            MapleInventoryManipulator.addById(mch.getClient(), 物品ID, (short)数量, "", null, (long)给予时间);
                        }
                    }
                    else {
                ...
                for (final MapleMapObject mmo : list) {
                    if (mmo != null) {
                        ...
                        if (chr.getId() == myId) {
                            ...
                            chr.getStat().setHp((int)chr.getStat().getMaxHp(),chr);
                            chr.updateSingleStat(MapleStat.HP, (int)chr.getStat().getMaxHp());
                            chr.getStat().setMp((int)chr.getStat().getMaxMp(),chr);
                    ...
                    if (mmo != null) {
                        ...
                        if (数量 >= 0) {
                            ...
                            if ((type.equals((Object)MapleInventoryType.EQUIP) && !GameConstants.isThrowingStar(物品ID) && !GameConstants.isBullet(物品ID)) || (type.equals((Object)MapleInventoryType.CASH) && 物品ID >= 5000000 && 物品ID <= 5000100)) {
                                ...
                                MapleInventoryManipulator.addById(chr.getClient(), 物品ID, (short)数量, "", null, (long)给予时间);
                            }
                        }
                        else {
    ...
    public String 等级排行榜() {
        ...
        try {
            ...
            final ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                if (rs.getInt("level") < (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"冒险家等级上限")) && rs.getInt("level") > 30) {
                    if (名次 < 10) {
    ...
    public String 满级排行榜() {
        ...
        try {
            ...
            final ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                if (rs.getInt("level") == (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"冒险家等级上限"))) {
                    final String 玩家名字 = rs.getString("name");
                    final String 职业 = this.职业(rs.getInt("job"));
