...
public class MaplePacketCreator{
    ...
    public static final byte[] getServerIP(final MapleClient c, final int port, final int clientId) {
        ...
        for (int a = 0; a < ServerConfig.IP.length; ++a) {
            if (MaplePacketCreator.初始ip.equals(ServerConfig.IP[a])) {
                具体个数 = a;
            }
        }
        ...
        if (ServerConfig.TESPIA) {
            ...
            mplew.write(ServerConfig.Gateway_IP[具体个数]);
        }
     ...
     * <获取经验,怪物>
     */
    public static final byte[] GainEXP_Monster(final int gain, final boolean white, final int wedding_EXP, final int partyinc, final int Class_Bonus_EXP, final int Equipment_Bonus_EXP, final int Premium_Bonus_EXP,final int 活动经验) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] giveForeignBuff(final int cid, final List<Pair<MapleBuffStat, Integer>> statups, final MapleStatEffect effect) {
        ...
        writeBuffState2(mplew, statups);
        for (final Pair<MapleBuffStat, Integer> statup : statups) {
            mplew.writeShort((int)((Integer)statup.getRight()).shortValue());
        }
        mplew.writeShort(0);
        if (effect.isMorph()) {
            mplew.write(0);
    ...
    public static byte[] itemEffects(int characterid, int itemid) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.SHOW_ITEM_GAIN_INCHAT.getValue());
        mplew.writeInt(characterid);
        mplew.writeInt(itemid);
