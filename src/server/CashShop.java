      ...
      if (period < 0) {
       ...
       boolean 加倍卡 = false;
        ...
        if (GameConstants.getInventoryType(cItem.getId()) == MapleInventoryType.EQUIP) {
            ...
            if (period > 0 || cItem.getPeriod_hour() > 0) {
                eq.setExpiration((long) (System.currentTimeMillis() + (long) (period * 24 * 60 * 60 * 1000L + cItem.getPeriod_hour() * 60 * 60 * 1000L)));
            } else {
                eq.setExpiration(-1);
            }
            ...
            if (period > 0) {
                item.setExpiration((long) (System.currentTimeMillis() + (long) (period * 24 * 60 * 60 * 1000 + cItem.getPeriod_hour() * 60 * 60 * 1000L)));
            } else {
