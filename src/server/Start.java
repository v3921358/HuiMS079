...
public class Start
{
    ...
    public static final void main(final String[] args) {
        if (Start.是否控制台启动) {
            ...
            System.out.println("◇ -> IP效验正在载入");
            boolean ip验证 = false;
            for (int a = 0; a < ServerConfig.IP.length; ++a) {
                if (MapleParty.IP地址.equals(ServerConfig.IP[a])) {
                    ip验证 = true;
                }
            }
            if (!ip验证) {
                System.out.println("◇ -> 当前IP不在允许使用范围");
            }else{
                System.out.println("◇ -> IP效验成功-正在加载其他项");
            World.init();
            ...
            System.out.println("◇ -> 加载游戏倍率信息");
            if (WorldConstants.EXP_RATE > 100.0) {
                System.out.println("◇ -> 游戏经验倍率: 100 倍 (上限)");
            }else {
                System.out.println("◇ -> 游戏经验倍率: " + WorldConstants.EXP_RATE + " 倍 ");
            }
            if (Integer.parseInt(ServerProperties.getProperty("HuiMs.dropRate")) > 100) {
                System.out.println("◇ -> 游戏物品倍率：100 倍 (上限)");
            }else {
                System.out.println("◇ -> 游戏物品倍率：" + Integer.parseInt(ServerProperties.getProperty("HuiMs.dropRate")) + " 倍 ");
            }
            if (Integer.parseInt(ServerProperties.getProperty("HuiMs.mesoRate")) > 100) {
                System.out.println("◇ -> 游戏金币倍率：100 倍 (上限)");
            }else {
                System.out.println("◇ -> 游戏金币倍率：" + Integer.parseInt(ServerProperties.getProperty("HuiMs.mesoRate")) + " 倍 ");
            }
            System.out.println("◇ -> 加载游戏端口配置");
            ...
            System.out.println("[泡点地图：910000000(自由市场) 2分钟/次]");
            福利泡点(2);
            ...
            System.out.println("[HuiMs服务端已启动完毕，耗时 " + (System.currentTimeMillis() - startQuestTime) / 1000L + " 秒]");
    ...
    public static int 进行授权校验(){
        int ret =0;
        String[] macs = {
            "64191a3e0235b1cf9fe1dd817a890048d1db84f2HuiMs",
            "3a475fdc398b70941df724c1700d33bac52f4bc7HuiMs",
            "3ed14855e0bf1cc4527bfd54a5d514924232a292HuiMs",
            "db4f74f62fe44ff0ed850be8cd1cfe82e39beb00",
            "d308cb4bc9ee82d0564a42ce4179e1358c62f0c4HuiMs",
        };
                String mac = MacAddressTool.getMacAddress(false);
                String num = returnSerialNumber();
                String localMac = LoginCrypto.hexSha1("xiao" + num + "hui" + mac + "hui") + "HuiMs";
        ...
        WorldTimer.getInstance().register((Runnable)new Runnable() {
            ...
            public void run() {
                if (记录在线时间 > 0) {
                    ...
                    if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"魔族突袭开关")) == 0 && calendar.get(11) == 22 && !(boolean)魔族入侵) {
                        活动魔族入侵.魔族入侵线程();
                        魔族入侵 = Boolean.valueOf(true);
                    }
                    if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"魔族攻城开关")) == 0 && Calendar.getInstance().get(7) == 1 && 时 == 21 && 分 <= 10 && !(boolean)魔族攻城) {
                        活动魔族攻城.魔族攻城线程();
                        魔族攻城 = Boolean.valueOf(true);
                    }
                    if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"幸运职业开关")) == 0) {
                    ...
                    if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"周末倍率开关")) == 0) {
                    ...
                    if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"神秘商人开关")) == 0) {
                    ...
                    if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"野外通缉开关")) == 0) {
                        if (初始通缉令 == 30) {
                            活动野外通缉.随机通缉();
                            初始通缉令++;
                        }
                ...
                try {
                    for (ChannelServer cserv : ChannelServer.getAllInstances()) {
                        for (MapleCharacter chr : cserv.getPlayerStorage().getAllCharacters()) {
                            ...
                            int expMod = 1;
                            int dropMod = 1;
                            if (chr.haveItem(5211060)) {//这个卡 经验2倍  基础经验
                                expMod = 3;
                            } else if (chr.haveItem(5211047)) {///这个卡 也两倍
                                expMod = 2;
                            }
                            if (chr.haveItem(5360015)) {//这个是爆率
                                dropMod = 2;
                            }
                            if (chr.haveItem(5360000)) {
                                dropMod = 2;
                                
                            }
                            chr.getStat().expMod = expMod;
                            chr.getStat().dropMod = dropMod;
                        }
                    }
                } catch (Exception e) {
                    System.err.println("重置经验爆率buff失败：" + e.getMessage());
                }
            }}, 1000);
    }
                    ...
                    try {
                        for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
                            for (final MapleCharacter chr : cserv.getPlayerStorage().getAllCharacters()) {
                                ...
                                int 抵用 = 0;
                                int 豆豆 = 0;
                                final int 泡点豆豆开关 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"泡点豆豆开关"));
                                if (泡点豆豆开关 <= 0) {
                                    final int 泡点豆豆 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"泡点豆豆"));
                                    豆豆 += 泡点豆豆;
                                    chr.gainBeans(豆豆);
                                }
                                final int 泡点金币开关 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"泡点金币开关"));
                                if (泡点金币开关 <= 0) {
                                    final int 泡点金币 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"泡点金币"));
                                    金币 += chr.getLevel() * 泡点金币;
                                    chr.gainMeso(chr.getLevel() * 泡点金币, true);
                                }
                                final int 泡点点券开关 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"泡点点券开关"));
                                if (泡点点券开关 <= 0) {
                                    final int 泡点点券 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"泡点点券"));
                                    chr.modifyCSPoints(1, 泡点点券, true);
                                    点券 += 泡点点券;
                                }
                                final int 泡点抵用开关 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"泡点抵用开关"));
                                if (泡点抵用开关 <= 0) {
                                    final int 泡点抵用 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"泡点抵用"));
                                    chr.modifyCSPoints(2, 泡点抵用, true);
                                    抵用 += 泡点抵用;
                                }
                                final int 泡点经验开关 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"泡点经验开关"));
                                if (泡点经验开关 > 0) {
                                    continue;
                                }
                                final int 泡点经验 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"泡点经验"));
        ...
        WorldTimer.getInstance().register((Runnable)new Runnable() {
            ...
            public void run() {
                ...
                for (ChannelServer cserv : ChannelServer.getAllInstances()) {
                    ...
                    for (MapleCharacter chr : cserv.getPlayerStorage().getAllCharacters()) {
                        ret += chr.saveToDB(false, false);//保存角色
                    }
                }
                //System.err.println("○【自动存档】 总共保存了" + ret + " 个角色数据 √");
            }
        }, (long)(60000 * time));
    }
    ...
    public static void 在线统计(final int time) {
        System.out.println("[HuiMs服务端启用在线统计." + time + "分钟统计一次在线的人数信息]");
        WorldTimer.getInstance().register((Runnable)new Runnable() {
            @Override
            public void run() {
