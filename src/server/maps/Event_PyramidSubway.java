...
public class Event_PyramidSubway
{
    ...
    public Event_PyramidSubway(final MapleCharacter c) {
        ...
        if (c.getParty() == null || c.getParty().getLeader().equals((Object)new MaplePartyCharacter(c))) {
            this.commenceTimerNextMap(c, 1);
            this.energyBarDecrease = MapTimer.getInstance().register((Runnable)new Runnable() {
                @Override
                public void run() {
    ...
    public final void commenceTimerNextMap(final MapleCharacter c, final int stage) {
        ...
        if (this.type != -1 && (stage == 4 || stage == 5)) {
            ...
            this.yetiSchedule = MapTimer.getInstance().register((Runnable)new Runnable() {
                @Override
                public void run() {
        ...
        this.timerSchedule = MapTimer.getInstance().schedule((Runnable)new Runnable() {
            @Override
            public void run() {
                boolean ret = false;
                if (type == -1) {
                    ...
                    ret = Event_PyramidSubway.warpNextMap_Pyramid(c, type);
                }
                if (!ret) {
                    Event_PyramidSubway.this.fail(c);
                }
    ...
    public final void broadcastUpdate(final MapleCharacter c) {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            ...
            c.getClient().sendPacket(MaplePacketCreator.sendPyramidUpdate(this.energybar));
        }
    }
    ...
    public static boolean warpStartSubway(final MapleCharacter c) {
        ...
        for (int i = 0; i < 5; ++i) {
            final MapleMap map = ch.getMapFactory().getMap(910320100 + i);
    ...
    public static boolean warpBonusSubway(final MapleCharacter c) {
        ...
        final ChannelServer ch = c.getClient().getChannelServer();
        for (int i = 0; i < 20; ++i) {
            final MapleMap map = ch.getMapFactory().getMap(910320010 + i);
            if (map.getCharactersSize() == 0) {
                clearMap(map, false);
