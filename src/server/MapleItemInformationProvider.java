...
public class MapleItemInformationProvider{
    ...
    public static int 弓标子弹叠加代码(final int itemId) {
        boolean 判定 = false;
        String 弓标子弹叠加代码[] = HuiMS.弓标子弹叠加上限突破.getText().split(",");
            for (int i = 0; i < 弓标子弹叠加代码.length; ++i) {
                if (itemId == Integer.parseInt(弓标子弹叠加代码[i])) {
                    判定 = true;
                }
            }
            if (判定) {
    ...
    public final short getSlotMax(final MapleClient c, final int itemId) {
        ...
        int 额外数量 = HuiMS.ConfigValuesMap.get("物品额外数量");
        int wz数量 = 0;
        if (item != null) {
            final MapleData smEntry = item.getChildByPath("info/slotMax");//取wz是否存在
            if (smEntry == null) {//不存在
                if(itemId >= 2070000 && itemId <= 2079999 || itemId >= 2060000 && itemId <= 2069999 || itemId >= 2330000 && itemId <= 2339999){//如果是标，弓，子弹
                    int 是否突破 = 弓标子弹叠加代码(itemId);
                    if(是否突破 == 1){
                        wz数量 = 1000;
                        ret = (short) (wz数量 + 额外数量);//数量就是1000+额外数量
                    }else{
                        ret = (short) 1000;//不存在标，弓，子弹上限就是1000  
                    }
                }else{//否则就不是标，弓，子弹
                ...
                if(itemId >= 2070000 && itemId <= 2079999 || itemId >= 2060000 && itemId <= 2069999 || itemId >= 2330000 && itemId <= 2339999){//如果是标，弓，子弹
                    ...
                    if(是否突破 == 1){
                        ...
                        ret = (short) MapleDataTool.getInt(smEntry);//如果存在，最大值就是wz里面的值
                    }
                }else{//否则就不是标，弓，子弹
                    if(MapleDataTool.getInt(smEntry) >=1000){//如果除了弓、标、子弹的wz数量大于等于1000
                        wz数量 = 1000;
                        ret = (short) (wz数量 + 额外数量);//数量就是1000+额外数量
                    }else{
                      ret = (short) (MapleDataTool.getInt(smEntry) + 额外数量); //如果wz小于1000，就是wz数量+额外数量
                    }
                }
            }
        }
