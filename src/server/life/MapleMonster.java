...
public class MapleMonster extends AbstractLoadedMapleLife{
    ...
    private void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final int numExpSharers, final byte pty, final byte classBounsExpPercent, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        if (attacker.level >= attacker.取限制等级()) {
            attacker.setExp(0);
            return;
        }
        if (highestDamage) {
        ...
        if (exp > 0) {
            if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"越级打怪开关")) == 0) {
            ...
            if (exp > 0) {
                ...
                if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"网吧经验加成")) != 0) {
                    final int 网吧经验加成 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"网吧经验加成"));
                ...
                //结婚经验加成
                int wedding_EXP = 0;
                if (attacker.getMarriageId() > 0 && attacker.getMap().getCharacterById_InMap(attacker.getMarriageId()) != null && (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"结婚经验加成")) != 0) {
                    final int 结婚经验加成 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"结婚经验加成"));
                    wedding_EXP = (int)((double)wedding_EXP + (double)exp / 100.0 * (double)结婚经验加成);
                }
                //人气经验加成
                if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"人气经验加成")) > 0 && attacker.getFame() > 0) {
                    attacker.人气经验加成();
                }
                attacker.gainExpMonster(exp, true, highestDamage, pty, wedding_EXP, classBonusExp, Equipment_Bonus_EXP, Premium_Bonus_EXP, lastskillID, getId(), stats.getLevel(), stats.getHp(),0);
            }
            attacker.mobKilled(this.getId(), lastskillID);
        }
    }
    private boolean 是否参与活动经验() {
        int ID = 0;
            try {
                ...
                System.err.println("[服务端]：雇佣活动经验出错");
            }
            return false;
    }
    public final int killBy(final MapleCharacter killer, final int lastSkill) {
        ...
        for (final AttackerEntry attackEntry : this.attackers) {
            final int baseExp = (int) Math.ceil(totalBaseExp * ((double) attackEntry.getDamage() / getMobMaxHp()));
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"怪物状态开关")) <= 0 && from.hasGmLevel(5)) {
            String 状态 = "";
