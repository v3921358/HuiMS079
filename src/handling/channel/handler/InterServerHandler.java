...
public class InterServerHandler{
    ...
    public static final void LoggedIn(final int playerid, final MapleClient c) {
        ...
        if (管理隐身 <= 0 && player.isGM()) {
            ...
            final int 管理加速 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"管理加速开关"));
            if (管理加速 <= 0) {
                if(player.getGMLevel() > 1){
        ...
        if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"上线提醒开关")) <= 0) {
        ...
        if ((int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"登陆帮助开关")) == 0) {
            if (player.getGMLevel() > 0 && player.getBossLog("管理上线提示") == 0) {
                player.dropMessage(5, "指令: [在HuiMs服务端内置] 查看管理员指令文本");
        ...
        if(c.getPlayer().TZ1 >= 套装1装备数量){
            int a;
            for(a = 0; a < c.getPlayer().TZ1 - 套装1装备数量;){
                a++;
            }
            c.getPlayer().dropMessage(-3, "【套装系统】：由于你穿戴套装系统组件 ["+ c.getPlayer().TZ1 + "件套] 附加"+ (套装1装备增加百分比 + 套装1一件附加 * a) +"%伤害");
        }
        if(c.getPlayer().TZ2 >= 套装2装备数量){
            int a;
            for(a = 0; a < c.getPlayer().TZ2 - 套装2装备数量;){
                a++;
            }
            c.getPlayer().dropMessage(-3, "【套装系统】：由于你穿戴套装系统组件 ["+ c.getPlayer().TZ2 + "件套] 附加"+ (套装2装备增加百分比 + 套装2一件附加 * a) +"%伤害");
        }
        if(c.getPlayer().TZ3 >= 套装3装备数量){
            int a;
            for(a = 0; a < c.getPlayer().TZ3 - 套装3装备数量;){
                a++;
            }
            c.getPlayer().dropMessage(-3, "【套装系统】：由于你穿戴套装系统组件 ["+ c.getPlayer().TZ3 + "件套] 附加"+ (套装3装备增加百分比 + 套装3一件附加 * a) +"%伤害");
        }
        if(c.getPlayer().TZ4 >= 套装4装备数量){int a;
            for(a = 0; a < c.getPlayer().TZ4 - 套装4装备数量;){
                a++;
            }
            c.getPlayer().dropMessage(-3, "【套装系统】：由于你穿戴套装系统组件 ["+ c.getPlayer().TZ4 + "件套] 附加"+ (套装4装备增加百分比 + 套装4一件附加 * a) +"%伤害");
        }
        if(c.getPlayer().TZ5 >= 套装5装备数量){
            int a;
            for(a = 0; a < c.getPlayer().TZ5 - 套装5装备数量;){
                a++;
            }
            c.getPlayer().dropMessage(-3, "【套装系统】：由于你穿戴套装系统组件 ["+ c.getPlayer().TZ5 + "件套] 附加"+ (套装5装备增加百分比 + 套装5一件附加 * a) +"%伤害");
        }
        if(c.getPlayer().TZ6 >= 套装6装备数量){
            int a;
            for(a = 0; a < c.getPlayer().TZ6 - 套装6装备数量;){
                a++;
            }
            c.getPlayer().dropMessage(-3, "【套装系统】：由于你穿戴套装系统组件 ["+ c.getPlayer().TZ6 + "件套] 附加"+ (套装6装备增加百分比 + 套装6一件附加 * a) +"%伤害");
        }
    }
    }
    ...
    static {
        InterServerHandler.离线挂机 = Boolean.parseBoolean(ServerProperties.getProperty("HuiMs.离线挂机"));
    }
}
