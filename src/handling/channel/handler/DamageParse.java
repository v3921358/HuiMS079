...
public class DamageParse{
        ...
        for (final AttackPair oned2 : attack.allDamage) {
            ...
            if (monster != null) {
                ...
                if (attack.skill != 1221011) {
                    monster.damage(player, (long)totDamageToOneMonster +  + 附加伤害总和, true, attack.skill);
                }else {
                ...
                if (player.getBuffedValue(MapleBuffStat.COMBO_DRAIN) != null) {
                    stats.setHp(stats.getHp() + (int)Math.min(monster.getMobMaxHp(), (long)Math.min((int)((double)totDamage * (double)player.getStatForBuff(MapleBuffStat.COMBO_DRAIN).getX() / 100.0), stats.getMaxHp() / 2)), true,player);
                }
                final int[] array;
                ...
                switch (attack.skill) {
                    ...
                    case 15111001: {
                        ...
                        stats.setHp(stats.getHp() + getHP, true,player);
                        break;
                    }
                    case 5211006:
                    case 5220011: {
                    ...
                    case 1311005: {
                        ...
                        stats.setHp((remainingHP > 1) ? remainingHP : 1,player);
                        break;
                    }
                    case 4001002:
                    case 4001334:
    ...
    public static final void applyAttackMagic(final AttackInfo attack, final ISkill theSkill, final MapleCharacter player, final MapleStatEffect effect) {
        ...
        if (attack.hits > 0 && attack.targets > 0 && !player.getStat().checkEquipDurabilitys(player, -1)) {
            player.dropMessage(5, "一个项目已经用完了耐久性，但没有库存空间可去。");
            return;
        }
        if (GameConstants.isMulungSkill(attack.skill)) {
            if (player.getMapId() / 10000 != 92502) {
