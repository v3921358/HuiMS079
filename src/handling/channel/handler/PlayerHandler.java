...
public class PlayerHandler
{
        ...
        if(c.getPlayer().吸怪特权 == 1){
            ...
            for (final MapleMonster monster : map.getAllMonsters()) {
                if(!monster.getStats().isBoss()){
                    ...
                    map.broadcastMessage(MobPacket.moveMonster(false, 0, 0, monster.getObjectId(), monster.getPosition(), map.getMonsterSpawnner().right, c.getPlayer().get吸怪Res()));
                }
            }
        }
    ...
    public static final void DropMeso(final int meso, final MapleCharacter chr) {
        final int 丢出金币开关 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"丢出金币开关"));
        if (丢出金币开关 > 0) {
    ...
    public static final void MovePlayer(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (res != null && c.getPlayer().getMap() != null) {
            ...
            final List<LifeMovementFragment> res2 = new ArrayList((Collection<? extends LifeMovementFragment>)res);
            ...
            for (int i = 0; i < clones.length; ++i) {
                if (clones[i].get() != null) {
                    final MapleCharacter clone = (MapleCharacter)clones[i].get();
                    final List<LifeMovementFragment> res3 = new ArrayList((Collection<? extends LifeMovementFragment>)res2);
                    CloneTimer.getInstance().schedule((Runnable)new Runnable() {
                        @Override
                        public void run() {
                            try {
                                if (clone.getMap() == map) {
                                    ...
                                    MovementParse.updatePosition(res3, (AnimatedMapleMapObject)clone, 0);
