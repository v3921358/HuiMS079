...
public class InventoryHandler
{
    ...
    public static final boolean UseRewardItem(final byte slot, final int itemId, final MapleClient c, final MapleCharacter chr) {
        ...
        if(ServerConfig.isxiangzi(toUse.getItemId())){//打开指定箱子调用npc
            c.sendPacket(MaplePacketCreator.enableActions());
            NPCScriptManager.getInstance().start(c, 9900004, ""+ toUse.getItemId()+"");
            return true;
        }else{
        if (toUse.getItemId() == MapleMap.吸怪盒子) {
            c.sendPacket(MaplePacketCreator.enableActions());
            NPCScriptManager.getInstance().start(c, 9900004, "吸怪特权");
            return true;
        }
        /*if (toUse.getItemId() == 2022670) {
            c.sendPacket(MaplePacketCreator.enableActions());
            NPCScriptManager.getInstance().start(c, 9900004, "黑龙箱子");
            return true;
        }*/
    ...
    public static final void UseCashItem(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (itemId) {
                    ...
                    switch (apfrom) {
                        ...
                        case MAXHP: {
                            ...
                            if (playerst.getHp() > playerst.getMaxHp()) {
                                playerst.setHp((int)maxhp,c.getPlayer());
                            }
                            playerst.setMaxHp(maxhp,c.getPlayer());
                            statupdate.put(MapleStat.MAXHP, Integer.valueOf((int)maxhp));
                        ...
                        case MAXMP: {
                            ...
                            if (playerst.getMp() > playerst.getMaxMp()) {
                                playerst.setMp((int)maxmp,c.getPlayer());
                            }
                            playerst.setMaxMp(maxmp,c.getPlayer());
                            statupdate.put(MapleStat.MAXMP, Integer.valueOf((int)maxmp));
            ...
            case 5570000: {
                ...
                if (随机数 <= ((Integer)HuiMS.ConfigValuesMap.get("金锤子提升成功率")).intValue()) {
            ...
            case 5070000: {
                final int 游戏喇叭 = (int)Integer.valueOf(HuiMS.ConfigValuesMap.get((Object)"游戏喇叭开关"));
                if (游戏喇叭 > 0) {
            ...
            case 5075005: {
                ...
                if (!MapleTVEffect.isActive()) {
                    MapleTVEffect mapleTVEffect = new MapleTVEffect(c.getPlayer(), victim, messages, tvType,time);
                    if (megassenger) {
                        builder.append(message);
                        World.Broadcast.broadcastSmega(MaplePacketCreator.serverNotice(3, c.getChannel(), c.getPlayer().getName() + " : " + builder.toString(), ear));
                    }
                    used = true;
                } else {
            ...
            case 5170000: {
                final MaplePet pet = c.getPlayer().getSummonedPet(0);
                final int slo = 0;
                if (pet == null) {
                    break;
                }
            ...
            case 5191004: {
                ...
                MaplePet pet2 = c.getPlayer().getSummonedPet(0);
                int slo2 = 0;
                if (pet2 == null) {
                    break;
                }
                if (pet2.getUniqueId() != uniqueid) {
                    pet2 = c.getPlayer().getSummonedPet(1);
                    slo2 = 1;
                    if (pet2 == null) {
                        break;
                    }
                    if (pet2.getUniqueId() != uniqueid) {
                        pet2 = c.getPlayer().getSummonedPet(2);
                        slo2 = 2;
                        if (pet2 == null) {
                            break;
                        }
    ...
    public static final void PetPickup(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        final MaplePet pet = chr.getSummonedPet((int)petz);
        slea.skip(1);
        chr.updateTick(slea.readInt());
        final Point Client_Reportedpos = slea.readPos();
        final int oid = slea.readInt();
    ...
    public static final void Owl(final LittleEndianAccessor slea, final MapleClient c) {
        if (c.getPlayer().haveItem(5230000, 1, true, false) || c.getPlayer().haveItem(2310000, 1, true, false)) {
            if (c.getPlayer().getMapId() >= 910000000 && c.getPlayer().getMapId() <= 910000022) {
                ...
                c.getPlayer().dropMessage(5, "猫头鷹只能在自由市场內使用");
                c.sendPacket(MaplePacketCreator.enableActions());
            }
        }
    }
    ...
    public static final void UseSkillBook(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        final int SuccessRate = ((Integer)skilldata.get("success")).intValue();
        final int ReqSkillLevel = ((Integer)skilldata.get("reqSkillLevel")).intValue();
        final int MasterLevel = ((Integer)skilldata.get("masterLevel")).intValue();
        byte i = 0;
        while (true) {
            final Integer CurrentLoopedSkillId = (Integer)skilldata.get("skillid" + (int)i);
            ++i;
            if (CurrentLoopedSkillId == null) {
                break;
            }
            if (Math.floor((double)(CurrentLoopedSkillId.intValue() / 10000)) != (double)chr.getJob()) {
                continue;
            }
            final ISkill CurrSkillData = SkillFactory.getSkill(CurrentLoopedSkillId.intValue());
    ...
    private static void 套装附加提示(MapleCharacter player, boolean 开关, Equip equip) {
        int 套装1装备数量 = HuiMS.ConfigValuesMap.get("套装1最少触发件数");//装备数量
                    int 套装1装备增加百分比 = HuiMS.ConfigValuesMap.get("套装1附加百分比伤害") ;//装备增加百分比
                    int 套装1一件附加 = HuiMS.ConfigValuesMap.get("套装1多穿一件附加");//附加
                    int 套装2装备数量 = HuiMS.ConfigValuesMap.get("套装2最少触发件数");//装备数量
                    int 套装2装备增加百分比 = HuiMS.ConfigValuesMap.get("套装2附加百分比伤害");//装备增加百分比
                    int 套装2一件附加 = HuiMS.ConfigValuesMap.get("套装2多穿一件附加");//附加
                    int 套装3装备数量 = HuiMS.ConfigValuesMap.get("套装3最少触发件数");//装备数量
                    int 套装3装备增加百分比 = HuiMS.ConfigValuesMap.get("套装3附加百分比伤害");//装备增加百分比
                    int 套装3一件附加 = HuiMS.ConfigValuesMap.get("套装3多穿一件附加");//附加
                    int 套装4装备数量 = HuiMS.ConfigValuesMap.get("套装4最少触发件数");//装备数量
                    int 套装4装备增加百分比 = HuiMS.ConfigValuesMap.get("套装4附加百分比伤害");//装备增加百分比
                    int 套装4一件附加 = HuiMS.ConfigValuesMap.get("套装4多穿一件附加");//附加
                    int 套装5装备数量 = HuiMS.ConfigValuesMap.get("套装5最少触发件数");//装备数量
                    int 套装5装备增加百分比 = HuiMS.ConfigValuesMap.get("套装5附加百分比伤害");//装备增加百分比
                    int 套装5一件附加 = HuiMS.ConfigValuesMap.get("套装5多穿一件附加");//附加
                    int 套装6装备数量 = HuiMS.ConfigValuesMap.get("套装6最少触发件数");//装备数量
                    int 套装6装备增加百分比 = HuiMS.ConfigValuesMap.get("套装6附加百分比伤害");//装备增加百分比
                    int 套装6一件附加 = HuiMS.ConfigValuesMap.get("套装6多穿一件附加");//附加
        if(HuiMS.ConfigValuesMap.get("套装系统开关") <=0){
            if(HuiMS.ConfigValuesMap.get("套装叠加开关") <= 0){//开启
                if(开关 && (player.getStat().自定义套装1(equip.getItemId()) 
                        || player.getStat().自定义套装2(equip.getItemId()) 
                        || player.getStat().自定义套装3(equip.getItemId()) 
                        || player.getStat().自定义套装4(equip.getItemId()) 
                        || player.getStat().自定义套装5(equip.getItemId()) 
                        || player.getStat().自定义套装6(equip.getItemId()))){
                    if(player.TZ1 >= 套装1装备数量){
                    int a;
                    for(a = 0; a < player.TZ1 - 套装1装备数量;){
                        a++;
                    }
                    //player.dropMessage(-3, "【套装系统】：由于你穿戴套装系统组件 ["+ player.TZ1 + "件套] 附加"+ (套装1装备增加百分比 + 套装1一件附加 * a) +"%伤害");
                    player.getClient().sendPacket(MaplePacketCreator.sendHint("【套装系统】：由于你穿戴套装系统组件 ["+ player.TZ1 + "件套] 附加"+ (套装1装备增加百分比 + 套装1一件附加 * a) +"%伤害", 350, 5));//头上显示
                    }
                if(player.TZ2 >= 套装2装备数量){
                    int a;
                    for(a = 0; a < player.TZ2 - 套装2装备数量;){
                        a++;
                    }
                    //player.dropMessage(-3, "【套装系统】：由于你穿戴套装系统组件 ["+ player.TZ2 + "件套] 附加"+ (套装2装备增加百分比 + 套装2一件附加 * a) +"%伤害");
                    player.getClient().sendPacket(MaplePacketCreator.sendHint("【套装系统】：由于你穿戴套装系统组件 ["+ player.TZ2 + "件套] 附加"+ (套装2装备增加百分比 + 套装2一件附加 * a) +"%伤害", 350, 5));//头上显示
                    
                }
                if(player.TZ3 >= 套装3装备数量){
                    int a;
                    for(a = 0; a < player.TZ3 - 套装3装备数量;){
                        a++;
                    }
                    //player.dropMessage(-3, "【套装系统】：由于你穿戴套装系统组件 ["+ player.TZ3 + "件套] 附加"+ (套装3装备增加百分比 + 套装3一件附加 * a) +"%伤害");
                    player.getClient().sendPacket(MaplePacketCreator.sendHint("【套装系统】：由于你穿戴套装系统组件 ["+ player.TZ3 + "件套] 附加"+ (套装3装备增加百分比 + 套装3一件附加 * a) +"%伤害", 350, 5));//头上显示
                }
                if(player.TZ4 >= 套装4装备数量){
                    int a;
                    for(a = 0; a < player.TZ4 - 套装4装备数量;){
                        a++;
                    }
                    //player.dropMessage(-3, "【套装系统】：由于你穿戴套装系统组件 ["+ player.TZ4 + "件套] 附加"+ (套装4装备增加百分比 + 套装4一件附加 * a) +"%伤害");
                    player.getClient().sendPacket(MaplePacketCreator.sendHint("【套装系统】：由于你穿戴套装系统组件 ["+ player.TZ4 + "件套] 附加"+ (套装4装备增加百分比 + 套装4一件附加 * a) +"%伤害", 350, 5));//头上显示
                }
                if(player.TZ5 >= 套装5装备数量){
                    int a;
                    for(a = 0; a < player.TZ5 - 套装5装备数量;){
                        a++;
                    }
                    //player.dropMessage(-3, "【套装系统】：由于你穿戴套装系统组件 ["+ player.TZ5 + "件套] 附加"+ (套装5装备增加百分比 + 套装5一件附加 * a) +"%伤害");
                    player.getClient().sendPacket(MaplePacketCreator.sendHint("【套装系统】：由于你穿戴套装系统组件 ["+ player.TZ5 + "件套] 附加"+ (套装5装备增加百分比 + 套装5一件附加 * a) +"%伤害", 350, 5));//头上显示
                }
                if(player.TZ6 >= 套装6装备数量){
                    int a;
                    for(a = 0; a < player.TZ6 - 套装6装备数量;){
                        a++;
                    }
                    //player.dropMessage(-3, "【套装系统】：由于你穿戴套装系统组件 ["+ player.TZ6 + "件套] 附加"+ (套装6装备增加百分比 + 套装6一件附加 * a) +"%伤害");
                    player.getClient().sendPacket(MaplePacketCreator.sendHint("【套装系统】：由于你穿戴套装系统组件 ["+ player.TZ6 + "件套] 附加"+ (套装6装备增加百分比 + 套装6一件附加 * a) +"%伤害", 350, 5));//头上显示
                }
                }else if(!开关 && player.getStat().自定义套装1(equip.getItemId()) 
                        || !开关 && player.getStat().自定义套装2(equip.getItemId()) 
                        || !开关 && player.getStat().自定义套装3(equip.getItemId()) 
                        || !开关 && player.getStat().自定义套装4(equip.getItemId()) 
                        || !开关 && player.getStat().自定义套装5(equip.getItemId()) 
                        || !开关 && player.getStat().自定义套装6(equip.getItemId())){
                    //player.dropMessage(-3, "【套装系统】：由于你脱掉套装系统组件，附加伤害减少了");
                    player.getClient().sendPacket(MaplePacketCreator.sendHint("【套装系统】：由于你脱掉套装系统组件，附加伤害减少了", 350, 5));//头上显示
                }
            }else{//不共存走这
                if(开关 && (player.getStat().自定义套装1(equip.getItemId()) && player.套装1是否共存 == true
                        || player.getStat().自定义套装2(equip.getItemId()) && player.套装2是否共存 == true
                        || player.getStat().自定义套装3(equip.getItemId()) && player.套装3是否共存 == true
                        || player.getStat().自定义套装4(equip.getItemId()) && player.套装4是否共存 == true
                        || player.getStat().自定义套装5(equip.getItemId()) && player.套装5是否共存 == true
                        || player.getStat().自定义套装6(equip.getItemId()) && player.套装6是否共存 == true)){
                    if(player.TZ1 >= 套装1装备数量){
                    int a;
                    for(a = 0; a < player.TZ1 - 套装1装备数量;){
                        a++;
                ...
                }else if((!开关 && player.getStat().自定义套装1(equip.getItemId())) && player.套装1是否共存 == true
                        || !开关 && player.getStat().自定义套装2(equip.getItemId()) && player.套装2是否共存 == true
                        || !开关 && player.getStat().自定义套装3(equip.getItemId()) && player.套装3是否共存 == true
                        || !开关 && player.getStat().自定义套装4(equip.getItemId()) && player.套装4是否共存 == true
                        || !开关 && player.getStat().自定义套装5(equip.getItemId()) && player.套装5是否共存 == true
                        || !开关 && player.getStat().自定义套装6(equip.getItemId()) && player.套装6是否共存 == true){
                    //player.dropMessage(-3, "【套装系统】：由于你脱掉套装系统组件，附加伤害减少了");
                    player.getClient().sendPacket(MaplePacketCreator.sendHint("【套装系统】：由于你脱掉套装系统组件，附加伤害减少了", 350, 5));//头上显示
                }
            }
    ...
    public static void SunziBF(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            try {
                ...
                try {
                    Item slotItem = (Item) c.getPlayer().getInventory(MapleInventoryType.USE).getItem(slot);
                    if (slotItem == null) {
                        return;
                    }
    ...
    public static void UseGachaExpHandler(final LittleEndianAccessor slea, final MapleClient c) {
        try {
