    ...
    static int gainStatByType(final MapleCharacter chr, final MapleStat type, final int gain) {
        if (chr != null) {
            ...
            if (type.equals((Object)MapleStat.STR)) {
                ...
                if (newVal > 32767) {
                    chr.getStat().setStr((short)32767,chr);
                }
                else {
                    chr.getStat().setStr(newVal,chr);
                }
            }
            else if (type.equals((Object)MapleStat.INT)) {
                newVal = (short)(chr.getStat().getInt() + gain);
                if (newVal > 32767) {
                    chr.getStat().setInt((short)32767,chr);
                }
                else {
                    chr.getStat().setInt(newVal,chr);
                }
            }
            else if (type.equals((Object)MapleStat.LUK)) {
                newVal = (short)(chr.getStat().getLuk() + gain);
                if (newVal > 32767) {
                    chr.getStat().setLuk((short)32767,chr);
                }
                else {
                    chr.getStat().setLuk(newVal,chr);
                }
            }
            else if (type.equals((Object)MapleStat.DEX)) {
                newVal = (short)(chr.getStat().getDex() + gain);
                if (newVal > 32767) {
                    chr.getStat().setDex((short)32767,chr);
                }
                else {
                    chr.getStat().setDex(newVal,chr);
                }
            }
            if (newVal > 32767) {
                chr.updateSingleStat(type, 32767);
                return newVal - 32767;
            }
