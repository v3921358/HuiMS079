...
public class World{
    ...
    public static void registerRespawn() {
        WorldTimer.getInstance().register((Runnable)new Respawn(), HuiMS.ConfigValuesMap.get("地图刷新频率"));
    }
    public static void registerRespawn1() {//备用刷怪线程
        WorldTimer.getInstance().register((Runnable)new Respawn1(), HuiMS.ConfigValuesMap.get("地图刷新频率"));
    }
            ...
            public void run() {
                ...
                if (rate.equals((Object)"经验")) {
                    for (final ChannelServer cservs : ChannelServer.getAllInstances()) {
                        cservs.setExpRate(Double.parseDouble(ServerProperties.getProperty("HuiMs.expRate")));
                    ...
                    for (final ChannelServer cservs : ChannelServer.getAllInstances()) {
                        cservs.setDropRate(Integer.parseInt(ServerProperties.getProperty("HuiMs.dropRate")));
                    ...
                    for (final ChannelServer cservs : ChannelServer.getAllInstances()) {
                        cservs.setMesoRate(Integer.parseInt(ServerProperties.getProperty("HuiMs.mesoRate")));
        ...
        if (numTimes % 100 == 0) {
            for (final MaplePet pet : chr.getSummonedPets()) {
                if (pet.getSummoned()) {
                    ...
                    int newFullness = 0;
                    if(chr.getItemQuantity(MapleMap.持有道具不饥饿,false) > 0){
