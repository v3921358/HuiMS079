...
public class ServerConstants
{
    ...
    public static void loadSetting() {
        final String[] x = ServerProperties.getProperty("HuiMs.cashshop.HotSell", "10000007, 10000008, 10000009, 10000010, 10000011").split(",");
        final int[] y = new int[x.length];
        for (int i = 0; i < x.length; ++i) {
            y[i] = Integer.parseInt(x[i].replace((CharSequence)" ", (CharSequence)""));
        }
        ServerConstants.hot_sell = y;
        ServerConstants.TESPIA = ServerProperties.getProperty("HuiMs.tespia", ServerConstants.TESPIA);
